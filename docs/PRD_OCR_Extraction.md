...
為確保 OCR 模組在第一階段能為業務員與管理員提供實質幫助，設定以下具體量化指標：

### 5.1 解析準確率與輔助效能
* [ ] **純文字版 PDF 自動辨識達標**：針對系統白名單內（全台前 20 大醫學中心）提供之「純文字 PDF」，L1~L3 混合引擎的全自動歸檔成功率需達 **>= 80%** (即不需人工介入)。
* [ ] **掃描檔/圖檔容忍度**：針對一般掃描版或影像截圖 (`source_type=image`)，系統至少需能透過定位擷取出醫師姓名，自動辨識率下限為 **>= 50%**。

### 5.2 管理員介入成本 (Admin SLA)
* [ ] **人工審核比例控制**：因 OCR 失敗或信心度不足而掉入 Pending 佇列的比例，不得超過整體上傳量的 **<= 20%**。
* [ ] **人工校正時間下限**：管理員使用「雙欄對照與區塊重新框選」介面修正單一有瑕疵的門診表時，平均操作時間需被壓縮在 **<= 3 分鐘** 以內（相比於全手動建檔的 10~15 分鐘）。
