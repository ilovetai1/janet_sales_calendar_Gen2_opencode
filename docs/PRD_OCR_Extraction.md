...
## 3. 解析技術選型評估 (Technical Options)
*(待討論確認)*
目前待評估的解析核心引擎：
- **方案 A (LLM Vision API)**：將 PDF 轉為高解析度圖片，交由 GPT-4o 或 Gemini 1.5 Pro 直出 JSON 格式。
- **方案 B (傳統 OCR + 規則引擎)**：先利用 AWS Textract 或 Google Cloud Vision 萃取文字區塊，再以後端強規則抓取。
- **方案 C (混合式架構 / Hybrid Engine)**：開源解析庫抓初稿，搭配防呆機制，最後無法判斷的區塊交由 LLM 或人工。
