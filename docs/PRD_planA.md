...
2. **加入關注清單**：醫師列表旁提供按鈕，點擊即可加入/移除「我的關注」。
3. **私密備忘錄 (Private Notes)**：使用者可針對關注醫師新增備忘錄 (例如：習慣喝什麼飲料、不喜歡被打擾的時段)，此筆記受到資料庫 RLS 保護，絕對僅限建立者本人可見。
4. **離線快取 (Offline Access)**：進入「我的關注」清單與查看備忘錄時，系統需支援離線存取，確保業務在醫院地下室診間等網路死角，仍可無縫查閱歷史筆記與目標名單。
