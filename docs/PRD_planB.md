...
**作為一個醫藥業務**，我希望能夠透過行政區或關鍵字搜尋某科別的醫師，點擊進入專屬的醫師個人頁面並將他們加入「我的關注」， **以便於**我能建立專屬於我的 Target 名單，淘汰紙本記事本。
* **[AC-1] 搜尋與探索**：首頁提供搜尋吧，支援輸入「醫院名稱」、「行政區」、「科別」或「醫師姓名」。
* **[AC-2] 醫師個人資訊頁 (Doctor Profile)**：點擊特定醫師後進入專屬頁面，統整呈現該名醫師最新的門診時段表。頁面需提供愛心或星號按鈕，可將其加入/移除「我的關注」。
* **[AC-3] Zero-Entry 筆記系統 (Smart Notes)**：在醫師個人頁面中，提供單一的大範圍私密備忘錄（例如記錄「習慣喝 50 嵐無糖綠」、「不喜歡被打擾的時段」），此筆記受資料庫 RLS 保護，絕對僅限建立者本人可見。
* **[AC-4] 離線快取 (Offline Access)**：進入「我的關注」清單與查看醫師個人頁面（含歷史備忘錄）時，APP 需支援完全離線存取，確保業務在醫院地下室診間等網路死角，仍可無縫查閱歷史筆記與目標名單。
