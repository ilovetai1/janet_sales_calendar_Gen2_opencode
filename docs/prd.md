...
#### 🎟️ Story 4: 鬆散的「時段重疊」警告機制 (Soft Conflict Warning)
**作為一個醫藥業務**，我希望在安排「下週排程」時，系統能提醒我如果同一天上午我安排了相距太遠的不同醫院， **以便於**我能及時發現行程不合理，但系統不應強制阻擋我排程。
* **[AC-1]** 當新增/匯入 Google 行事曆行程時，系統比對該使用者的既有行程。
* **[AC-2]** 只要「同一天」內的「同一個大時段（上午/下午/夜間）」出現了兩家「不同醫院」的行程，介面上會跳出「⚠️ 醫院跨區時段重疊警告」。
* **[AC-3]** 警告僅作視覺提示，使用者仍可點擊「確認儲存」強行排入行程。
